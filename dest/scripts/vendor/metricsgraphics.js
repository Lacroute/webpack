            ...
            if (continueWithDefault !== false) {
                for (var i = args.data.length - 1; i >= 0; i--) {
                    ...
                    if (displayArea) {
                        ...
                        if (!areas.empty()) {
                          // console.log(svg.select('.mg-y-axis')[0][0]);
                          if(svg.select('.mg-y-axis')[0][0] !== null)
                ...
                if (args.show_rollover_text) {
                    ...
                    if (args.aggregate_rollover && args.data.length > 1) {
                        ...
                        if (args.time_series) {
                            ...
                            // console.log("time series loop without aggregate_rollover 1");
                            if (args.rollover_text_export.node === '') {
                              textContainer.append('tspan')
                                  .text(fmt(dd) + '  ' + args.yax_units
                                      + num(d[args.y_accessor]));
                            } else {
                              var aMonthAgo, aYearAgo,
                                  textTarget1 = d3.select(args.rollover_text_export.node[0]),
                                  textTarget2 = d3.select(args.rollover_text_export.node[1]),
                                  sFormat = d3.time.format('%B %Y'),
                                  nFormat = num(d[args.y_accessor]).replace(/,/g, ''),
                                  // nResult = nFormat.substring(0, 1) + ' ' + nFormat.substring(1, 4) + ' ' + nFormat.substring(4);
                                  nResult = parseInt(nFormat, 10).toLocaleString('fr-FR'),
                                  localString = window.AFP_translation;

                              _.forEach(window.datas, function(n, i) {
                                // console.log(n[args.y_accessor], parseInt(nFormat));
                                if (n[args.y_accessor] === parseInt(nFormat, 10)) {
                                  // console.log("I ", i);
                                  aMonthAgo = (i > 0) ? i - 1 : 0;
                                  aYearAgo = (i > 12) ? i - 12 : 0;
                                }
                              });



                              var progMonth = (parseInt(nFormat, 10) - window.datas[aMonthAgo][args.y_accessor]).toLocaleString('fr-FR'),
                                  progYear = (parseInt(nFormat, 10) - window.datas[aYearAgo][args.y_accessor]).toLocaleString('fr-FR');
                              // console.log("2 dates ", progMonth, progYear, parseInt(nFormat, 10));

                              if (parseInt(progMonth, 10) > 0) progMonth = '+' + progMonth;
                              if (parseInt(progYear, 10) > 0) progYear = '+' + progYear;


                              // textTarget.html(args.x_accessor + ': ' + d[args.x_accessor]
                              //           + ', ' + args.y_accessor + ': ' + args.yax_units
                              //           + num(d[args.y_accessor]));
                              textTarget1.html('<span class="date">' + sFormat(d[args.x_accessor]) + '</span><span class="big-number">'+ nResult + '</span><span class="cat-rollover">'+ localString.rolloverBlock.unemployed +'</span>');
                              textTarget2.html(progMonth + '<span>'+ localString.rolloverBlock.about_1_month +'</span>' + progYear +'<span>'+ localString.rolloverBlock.about_1_year +'</span>');
                            }
                        }
                        else {
                            console.log("time series loop without aggregate_rollover 2");
                            if (args.rollover_text_export.node === '') {
                                textContainer.append('tspan')
                                ...
                                console.log(typeof args.rollover_text_export);
                                var textTarget = d3.select(args.rollover_text_export.node);
                                // console.log(textTarget);
                                //
                                textTarget.append('tspan')
                                    .text(args.x_accessor + ': ' + d[args.x_accessor]
                                        + ', ' + args.y_accessor + ': ' + args.yax_units
                                        + num(d[args.y_accessor]));
                            }
        ...
        this.rolloverOff = function(args) {
            ...
            return function(d, i) {
                ...
                if (args.rollover_text_export.node === '') {
                    ...
                    // d3.select(args.rollover_text_export.node[0]).html('');
                    // d3.select(args.rollover_text_export.node[1]).html('');
                }
