...
document.addEventListener("DOMContentLoaded", function(event) {
  "use strict";
  // console.log("check MG existenz ", window.MG);
  ...
  // });
  queue()
    .defer(d3.json, "data/fr-FR/fr-d3-formatters.json")
  ...
  function drawGraph(lineOptions) {
    ...
    window.MG.data_graphic(updateSettings);
  }
  ...
  function initNewGraph(arrKeys, arrRange) {
    ...
    // simulate clic to reveal circle
    // if (once) {
    //   once = false;
  ...
  // calling datas
  queue()
    .defer(d3.json, 'data/fr-FR/fr-project.json')
    ...
    .await(function(err, data1, data2) {
      ...
      d3.select('.filters-link').attr('class', 'filters-link active');
      ...
      initNewGraph(['month', 'noActivity'], [window.datas.length-6, window.datas.length]);
    });
});
